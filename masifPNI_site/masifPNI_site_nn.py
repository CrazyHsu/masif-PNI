...
# -*- coding: utf-8 -*-
'''
File name: masifPNI_site_nn.py
Author: CrazyHsu @ crazyhsu9627@gmail.com
Created on: 2022-09-12 16:24:22
Last modified: 2022-09-12 16:24:22
'''
...
class MasifPNI_site_nn:
    ...
    def inference(
        ...
        for k in range(self.n_rotations):
            ...
            gauss_activations = tf.reshape(gauss_activations, [n_samples, n_vertices, -1])  # batch_size, n_vertices, n_gauss
            ...
            if (mean_gauss_activation):  # computes mean weights for the different gaussians
                gauss_activations /= (tf.reduce_sum(gauss_activations, 1, keep_dims=True) + eps)  # batch_size, n_vertices, n_gauss
            ...
            gauss_desc = tf.reshape(gauss_desc, [n_samples, self.n_thetas * self.n_rhos * n_feat])  # batch_size, self.n_thetas*self.n_rhos*n_feat
    ...
    def __init__(
        ...
        with tf.Graph().as_default() as g:
            ...
            with tf.device(idx_xpu):
                ...
                self.rho_coords = tf.placeholder(tf.float32)  # batch_size, n_vertices, 1
                self.theta_coords = tf.placeholder(tf.float32)  # batch_size, n_vertices, 1
                self.input_feat = tf.placeholder(tf.float32, shape=[None, None, self.n_feat])  # batch_size, n_vertices, n_feat
                ...
                self.indices_tensor = tf.placeholder(tf.int32)  # batch_size, max_verts (< 30)
...
class MasifPNI_site_nn1:
    ...
    def inference(
        ...
        n_feat = tf.shape(input=input_feat)[2]
        ...
        for k in range(self.n_rotations):
            ...
            rho_coords_ = tf.exp(-tf.square(rho_coords_ - mu_rho) / (tf.square(sigma_rho) + eps))
            thetas_coords_ = tf.exp(-tf.square(thetas_coords_ - mu_theta) / (tf.square(sigma_theta) + eps))
            self.myshape = tf.shape(input=rho_coords_)
            self.rho_coords_debug = rho_coords_
            self.thetas_coords_debug = thetas_coords_
            ...
            gauss_activations = tf.multiply(gauss_activations, mask)
            if (mean_gauss_activation):  # computes mean weights for the different gaussians
                gauss_activations /= (tf.reduce_sum(input_tensor=gauss_activations, axis=1, keepdims=True) + eps)  # batch_size, n_vertices, n_gauss
    ...
    def compute_data_loss(self, neg_thresh=1e1):
        ...
        neg_thresh = 0.0
        pos_labels = self.labels[:, 0]
        n_pos = tf.reduce_sum(input_tensor=pos_labels)
        pos_scores = tf.multiply(self.logits[:, 0], tf.cast(pos_labels, dtype=tf.float32))
        pos_scores = tf.reduce_sum(input_tensor=pos_scores) / n_pos

        neg_labels = self.labels[:, 1]
        n_neg = tf.reduce_sum(input_tensor=neg_labels)
        neg_scores = tf.multiply(self.logits[:, 1], tf.cast(neg_labels, dtype=tf.float32))
        neg_scores = tf.reduce_sum(input_tensor=neg_scores) / n_neg
    ...
    def __init__(
        ...
        with tf.Graph().as_default() as g:
            ...
            with tf.device(idx_xpu):
                ...
                self.rho_coords = tf.compat.v1.placeholder(tf.float32)  # batch_size, n_vertices, 1
                self.theta_coords = tf.compat.v1.placeholder(tf.float32)  # batch_size, n_vertices, 1
                self.input_feat = tf.compat.v1.placeholder(tf.float32, shape=[None, None, self.n_feat])  # batch_size, n_vertices, n_feat
                ...
                self.indices_tensor = tf.compat.v1.placeholder(tf.int32)  # batch_size, max_verts (< 30)
                ...
                for i in range(self.n_feat):
                    b_conv.append(
                        tf.Variable(
                            tf.zeros([self.n_thetas * self.n_rhos]),
                            name="b_conv_{}".format(i), trainable=True
                        )
                    )
                for i in range(self.n_feat):
                    my_input_feat = tf.expand_dims(self.input_feat[:, :, i], 2)
                ...
                if n_conv_layers > 1:
                    ...
                    batch_size = tf.shape(input=self.global_desc)[0]
                    # Reduce the dimensionality by averaging over the last dimension
                    self.global_desc = tf.reshape(
                        self.global_desc,
                        [batch_size, self.n_feat, self.n_thetas * self.n_rhos],
                    )
                    self.global_desc = tf.reduce_mean(input_tensor=self.global_desc, axis=2)
                    self.global_desc_shape = tf.shape(input=self.global_desc)
                ...
                if n_conv_layers > 3:
                    ...
                    self.global_desc = tf.reduce_max(input_tensor=self.global_desc, axis=2)
                    self.global_desc_shape = tf.shape(input=self.global_desc)
                # refine global desc with MLP
                self.global_desc = tf.compat.v1.layers.dense(
